...
static string BuildSelectAllQuery(const SnowflakeConfig &config, const string &schema_name, const string &table_name) {
	// Matches the existing call sites in this file: no extra quoting, since the
	// rest of the extension at v1.5.2 doesn't quote identifiers either.
	return "SELECT * FROM " + config.database + "." + schema_name + "." + table_name;
}
